...
def cosine_annealing(start_factor: float = 1.0, end_factor: float = 0.0, time_scale: float = 1.0, **kwargs) -> Callable[[float], float]:
...
def linear_decay(start_factor: float = 1.0, end_factor: float = 0.0, time_scale: float = 1.0, **kwargs) -> Callable[[float], float]:
    def schedule(t: float) -> float:
        if t < time_scale:
...
def get_schedule(config: Dict[str, Any]) -> Callable[[float], float]:
    ...
    schedule_name = config.get("name", None)
