...
def gather_metrics(data):
    ...
    for checkpoint in checkpoints:
        ...
        for dataset in datasets:
            ...
            dataset_harmonic_mean = harmonic_mean([v for v in dataset_metrics.values() if not np.isnan(v)])
...
def plot_metrics(gathered_data, output_dir):
    ...
    fig, axs = plt.subplots(n_datasets, 3, figsize=(24, 6 * n_datasets), sharex=True)
    if n_datasets == 1:
        axs = axs.reshape(1, -1)

    colors = ['red', 'blue', 'green']
    labels = ['Harmonic Mean', 'ROUGE_L', 'Probability (Log Scale)']

    for i, dataset in enumerate(datasets):
        for j, (metric, color, label) in enumerate(zip(['harmonic_mean'] + metrics, colors, labels)):
...
def plot_metrics_across_checkpoints(data, output_dir):
    """
    Example input format:
    data = {
        "checkpoint-200": {
            "dataset1": {"rouge_l": 0.55, "probability": 0.05},
            "dataset2": {"rouge_l": 0.77, "probability": 0.43},
        },
        ...
    }
    """
    os.makedirs(output_dir, exist_ok=True)
    gathered_data = gather_metrics(data)
...
if __name__ == "__main__":
    ...
    metrics_plot, overall_plot = plot_metrics_across_checkpoints(data, args.output_dir)
